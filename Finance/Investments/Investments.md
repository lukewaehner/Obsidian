**Focus**: Financial markets, investment analysis, portfolio management, and derivatives pricing with emphasis on quantitative methods and risk management.

##  Core Topics

### Fundamental Concepts

#### [[Risk & Return]]
Understanding the risk-return tradeoff
- Expected returns and risk measurement
- Standard deviation and variance
- Risk premium
- Sharpe ratio and risk-adjusted returns
- Systematic vs unsystematic risk
- Beta and market risk
- Risk aversion and utility theory

#### [[Asset Classes]]
Different investment categories and their characteristics
- **Equities**: Stocks, ownership stakes
- **Fixed Income**: Bonds, credit instruments
- **Cash Equivalents**: Money market instruments
- **Derivatives**: Options, futures, swaps
- **Alternative Investments**: Real estate, commodities, private equity
- Asset class correlations and diversification

### Valuation & Analysis

#### [[Valuation Metrics]]
Methods for pricing and evaluating investments
- **Equity Valuation**:
  - P/E ratio, P/B ratio
  - Dividend discount model (DDM)
  - DCF (discounted cash flow)
  - Comparable company analysis
- **Bond Valuation**:
  - Present value of cash flows
  - Yield to maturity (YTM)
  - Duration and convexity
  - Credit spreads
- **Relative Valuation**: Multiples and comparables

### Portfolio Management

#### [[Investment Process]]
Systematic approach to investing
- **Planning**: Goals, constraints, time horizon
- **Asset Allocation**: Strategic and tactical allocation
- **Security Selection**: Picking individual investments
- **Execution**: Trading and implementation
- **Monitoring**: Performance tracking and rebalancing
- **Evaluation**: Performance attribution and analysis

#### [[Capital Allocation]]
Optimal portfolio construction
- **Portfolio Theory**: Markowitz mean-variance optimization
- **Efficient Frontier**: Risk-return combinations
- **Capital Market Line (CML)**: Risk-free asset + market portfolio
- **Security Market Line (SML)**: CAPM relationship
- **Diversification Benefits**: Reducing portfolio risk
- **Optimal Risky Portfolio**: Tangency portfolio
- **Two-Fund Separation**: Risk-free asset + risky portfolio

### Derivatives & Advanced Topics

#### [[Derivatives|Derivatives]]
Contingent claims and risk management instruments

**Futures Contracts**:
- Forward vs futures
- Hedging strategies
- Basis risk
- Pricing and arbitrage
- Commodity and financial futures

**Options**:
- Call and put options
- Option pricing (Black-Scholes)
- Greeks (delta, gamma, theta, vega)
- Option strategies:
  - Covered call
  - Protective put
  - Straddle and strangle
  - Spreads (bull, bear, butterfly)
- American vs European options

**Swaps**:
- Interest rate swaps
- Currency swaps
- Valuation and uses

## Key Financial Models

### Capital Asset Pricing Model (CAPM)
```
E(Ri) = Rf + βi[E(Rm) - Rf]

Where:
- E(Ri) = Expected return on asset i
- Rf = Risk-free rate
- βi = Beta of asset i
- E(Rm) = Expected market return
```

### Bond Pricing
```
Price = Σ [C / (1 + r)^t] + [F / (1 + r)^T]

Where:
- C = Coupon payment
- r = Discount rate/YTM
- F = Face value
- T = Maturity
```

### Black-Scholes Option Pricing
```
Call = S₀N(d₁) - Ke^(-rT)N(d₂)
Put = Ke^(-rT)N(-d₂) - S₀N(-d₁)

Where d₁, d₂ involve stock price, strike, volatility, time, rate
```

## Investment Concepts

### Modern Portfolio Theory
- **Diversification**: Reducing unsystematic risk
- **Efficient Frontier**: Optimal risk-return portfolios
- **Market Portfolio**: All risky assets in market proportions
- **Separation Theorem**: All investors hold same risky portfolio

### Market Efficiency
- **Weak Form**: Past prices don't predict future
- **Semi-Strong Form**: Public info instantly reflected
- **Strong Form**: All info (including private) reflected

### Behavioral Finance
- Cognitive biases in investing
- Market anomalies
- Limits to arbitrage
- Investor psychology

## Practical Applications

### Portfolio Construction
1. **Determine Investment Policy**:
   - Return objectives
   - Risk tolerance
   - Time horizon
   - Liquidity needs
   - Tax considerations

2. **Asset Allocation**:
   - Strategic (long-term)
   - Tactical (short-term adjustments)
   - Rebalancing rules

3. **Security Selection**:
   - Top-down vs bottom-up
   - Fundamental analysis
   - Technical analysis
   - Quantitative screens

4. **Implementation & Monitoring**:
   - Trade execution
   - Cost minimization
   - Performance tracking
   - Risk management

### Risk Management
- **Value at Risk (VaR)**: Potential loss estimation
- **Stress Testing**: Extreme scenario analysis
- **Hedging**: Using derivatives to reduce risk
- **Stop-Loss**: Limiting downside
- **Position Sizing**: Risk per trade

## Financial Instruments Deep Dive

### Fixed Income Securities
- **Government Bonds**: Treasury bills, notes, bonds
- **Corporate Bonds**: Investment grade, high yield
- **Municipal Bonds**: Tax-advantaged
- **Bond Characteristics**:
  - Coupon rate
  - Maturity date
  - Credit rating
  - Call/put provisions

### Equity Securities
- **Common Stock**: Voting rights, dividends
- **Preferred Stock**: Fixed dividends, priority
- **ADRs**: Foreign companies on US exchanges
- **Equity Analysis**:
  - Fundamental analysis
  - Technical analysis
  - Quantitative factors

### Derivative Applications
- **Hedging**: Reducing risk exposure
- **Speculation**: Leveraged directional bets
- **Arbitrage**: Exploiting pricing inefficiencies
- **Income Generation**: Writing covered calls

## Quantitative Methods

### Statistical Tools
- Regression analysis
- Correlation and covariance
- Time series analysis
- Monte Carlo simulation
- Optimization techniques

### Performance Metrics
- **Return Measures**:
  - Arithmetic vs geometric returns
  - Time-weighted vs money-weighted
  - Total return vs price return

- **Risk-Adjusted Measures**:
  - Sharpe ratio: (Return - Rf) / σ
  - Treynor ratio: (Return - Rf) / β
  - Jensen's alpha: Actual - CAPM expected
  - Information ratio: Active return / tracking error

## Learning Objectives

### Core Competencies
1. **Understand** risk-return tradeoffs
2. **Analyze** securities using valuation models
3. **Construct** diversified portfolios
4. **Apply** derivatives for hedging and speculation
5. **Evaluate** investment performance
6. **Manage** portfolio risk

### Quantitative Skills
- Bond math and duration
- Option pricing models
- Portfolio optimization
- Performance attribution
- Risk measurement

### [[Exercises]]
Problem sets and practice problems
- Bond pricing calculations
- Option strategy payoffs
- Portfolio optimization
- Derivatives valuation
- Risk metrics computation

### Real-World Application
- Personal portfolio management
- Analyzing market data
- Derivatives trading strategies
- Risk hedging techniques
- Performance evaluation

## Career Connections

### Fintech Focus
- **Order Matching Engines**: High-performance trading systems
- **ETF Data Collection**: Automated financial data pipelines
- **Portfolio Analytics**: Investment analysis tools
- **Risk Systems**: Real-time risk monitoring

### Relevant Experience
- **Riverside Financial Intern**: ETF data automation (70% time reduction)
- **Personal Projects**: Rust order book (5.7M+ orders/sec, 13μs latency)
- **Technical Skills**: Systems programming for finance

---

**Topics**: Investments, Portfolio Management, Derivatives, Fixed Income, Equity Analysis, Risk Management
**Focus Areas**: Quantitative Methods, Financial Engineering, Fintech Applications

---

### Links
[[Asset Classes]]
[[Behavioral Finance]]
[[Capital Allocation]]
[[Derivatives]]
[[Efficient Diversification]]
[[Archive/Finance/Investments/Efficient Market Hypothesis/Efficient Market Hypothesis|Efficient Market Hypothesis]]
[[Investment Process]]
[[Risk & Return]]
[[Valuation Metrics]]
[[Capital Asset Pricing Model]]

```folder-overview
id: 265341d5-0e95-41ad-97d6-3c43eacbce93
folderPath: Finance/Investments
title: "{{folderName}} overview"
showTitle: false
depth: 1
includeTypes:
  - folder
  - markdown
style: list
disableFileTag: false
sortBy: name
sortByAsc: true
showEmptyFolders: false
onlyIncludeSubfolders: false
storeFolderCondition: true
showFolderNotes: false
disableCollapseIcon: true
alwaysCollapse: false
autoSync: true
allowDragAndDrop: true
hideLinkList: true
hideFolderOverview: false
useActualLinks: true
fmtpIntegration: false
titleSize: 1
isInCallout: false
useWikilinks: true
```
<span class="fv-link-list-start" id="265341d5-0e95-41ad-97d6-3c43eacbce93"></span>
<span class="fv-link-list-end" id="265341d5-0e95-41ad-97d6-3c43eacbce93"></span>
