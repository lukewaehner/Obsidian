---
type: topic
group: Algorithm Design
tier: extra
confidence:
---

# Linear Programming

> [!abstract]- Coverage — 0/6
> - [ ] [[#Idea]]
> - [ ] [[#How it works]]
> - [ ] [[#Implementation]]
> - [ ] [[#Complexity]]
> - [ ] [[#When to use it]]
> - [ ] [[#Gotchas]]

## Idea

Optimize a linear objective subject to linear constraints — the simplex
method walks the vertices of the feasible polytope.

## How it works

## Implementation

## Complexity

## When to use it

## Gotchas

## Resources

- [Linear Programming (video)](https://www.youtube.com/watch?v=M4K6HYLHREQ)
- [Finding minimum cost (video)](https://www.youtube.com/watch?v=2ACJ9ewUC6U)
- [Finding maximum value (video)](https://www.youtube.com/watch?v=8AA_81xI3ik)
- [Solve Linear Equations with Python - Simplex Algorithm (video)](https://www.youtube.com/watch?v=44pAWI7v5Zk)

## Problems

_None yet._
